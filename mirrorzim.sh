...
ZIM_FILE_SOURCE_URL="$(./tools/getzim.sh download $WIKI_TYPE $WIKI_TYPE $LANGUAGE_CODE top mini latest | grep 'URL:' | cut -d' ' -f3)"
...
if [[ "$PUSH" == "true" ]]; then
    ./tools/add_to_ipfs.sh "$ZIM_FILE" "$TMP_DIRECTORY"
else
    printf "\n\n-------------------------\nD O N E !\n-------------------------\n"
    printf "ZIM: $ZIM_FILE\n"
